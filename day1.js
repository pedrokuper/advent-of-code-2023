const str = `nkzjrdqrmpztpqninetwofour1znnkd
...
for (let i = 0; i < arrayOfStrings.length; i++) {
  ...
  for (let j = 0; j < arrayOfStrings[i].length; j++) {
    const convertToNumber = parseInt(arrayOfStrings[i][j]);
    if (!Number.isNaN(convertToNumber)) {
      auxArray.push(convertToNumber);
    }
  }
  finalArray.push([auxArray[0], auxArray[auxArray.length - 1]]);
}

let sum = 0;
for (let i = 0; i < finalArray.length; i++) {
  sum += parseInt(finalArray[i].toString().replace(",", ""));
}
